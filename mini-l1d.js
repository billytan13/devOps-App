...
{
    connection.query('SELECT * FROM Ugrp WHERE Grpname = ? and Uname = ?', [gnam, unam], function( err, result) 
    {
        if (err) 
            callback(err,null);
        else
...
function GetMyDate() { 
	...
	mDate= myDate.getFullYear()+'-'+myM+'-'+myDate.getDate() +'-'+myDate.getHours() +':'+myDate.getMinutes()+':'+myDate.getSeconds();
...
function stamp(username,mCrState) {
	var myStamp="";
	myStamp="by ["+username+"] @ "+GetMyDate()+" [state:"+mCrState+"]";
	return(myStamp);
  }
...
function sonarResult(sonaQ, sonaU, sonaP, relname,callback )
{
	...
	const request=require('request');
		request('http://'+sonaU+":"+sonaP+'@'+sonaQ+'/api/qualitygates/project_status?projectKey='+relname , function (err, res, body) {
...
app.get('/changepassword', (req, res) => {
	if (req.session.isLoggedIn === true) {
		connection.query('SELECT * FROM accounts where username=?',[req.session.username], function(err, rows, fields) {
			if (err) {
				res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
			} else {
				var appList=[];
			...
			res.render('changepassword', {"app": app, isLoggedIn: req.session.username});
			}
	  });	
	}
	else
...
app.get('/changepassword/:Uname', (req, res) => {
	if (req.session.isLoggedIn === true) {
		connection.query('SELECT * FROM accounts where username=?',[req.params.Uname], function(err, rows, fields) {
			if (err) {
				res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
			} else {
				var appList=[];
			...
			// Render index.pug page using array 
			res.render('updateaccount', {"app": app, isLoggedIn: req.session.username});
			}
	  });	
	}
	else
...
app.post('/changepassword', (req, res) =>{
	...
	console.log(rever);
	if (Uname && Upass) {
		connection.query('update accounts set password=?, email=? where username = ? AND password = ?', [hash1,Uemail, Uname, hash], function(err, result) {
			if (err) 
				res.render('login', {error: 'Unable to change pasword as current password is incorrect.'});
			else
			    {
				  if (result.changedRows===0)
				      res.render('index', {error: 'Unable to change password as current password is incorrect.', isLoggedIn: req.session.username});
				  else
   			    	  res.render('index', {isLoggedIn: req.session.username});
				}
			});
		...
		getPermit("admin",req.session.username, function(err,data){
			if (err) { console.log("error adding user"+err);
			    ...
			    if (data===1) {
					...
					res.render('index', {error: "You have no admin rights.", isLoggedIn: req.session.username});
				    }
				}
	  });	
	}
...
app.post('/user_add', (req, res) =>{
	...
	hash=getcrypt(Upass);
	if (Uname && Upass) {
		connection.query(' insert into `accounts` (`email`,`username`,`password`) values (?,?,?)', [Uemail, Uname, hash], function(err, result) {
			if (err) 
				res.render('user_add', {error: 'Create user failed:'+err, isLoggedIn: req.session.username});
			else
			    res.render('user_add', {error: 'User Created:'+Uname, isLoggedIn: req.session.username});
			});
	} 
	else {
    	res.render('login', {error: 'Username or password is incorrect'});
			...
			if (err) { console.log("error adding user"+err);
			    ...
			    if (data===1) {
					...
					connection.query('SELECT * FROM accounts', function(err, rows, fields) {
						if (err) {
							res.status(500).json({"status_code": 500,"status_message": "internal server error@user_manage"});
						} else {
							// Loop check on each row
							for (var i = 0; i < rows.length; i++) {			  
								// Create an object to save current row's data
						...
						res.render('user_manage', {"uList": uList, isLoggedIn: req.session.username});
						}
					});	
					}
				else{
...
app.post('/changepassword/update_account', (req, res) => {
	...
	hash=getcrypt(Upass);
	if (Uname && Upass) {
		connection.query('update accounts set password=?, email=? where username = ?', [hash,Uemail, Uname], function(err, result) {
			if (err) 
				res.render('index', {error: 'Unable to update account'+err});
			else
			    {
				  if (result.changedRows===0)
				      res.render('index', {error: 'No row updated', isLoggedIn: req.session.username});
				  else
   			    	  res.render('index', {isLoggedIn: req.session.username});
				}
			});
...
app.post('/user_group_add', (req, res) =>{
	...
	if (Uname && Gname) {
		connection.query(' insert into `ugrp` (`Grpname`,`Uname`) values (?,?)', [Gname, Uname], function(err, result) {
			if (err) 
				res.render('user_group_add', {error: 'Create group failed:'+err, isLoggedIn: req.session.username});
			else
			    res.render('user_group_add', {error: 'Group Created:   '+Gname+" - "+Uname, isLoggedIn: req.session.username});
			});
	} 
	else {
			...
			if (err) { console.log("error adding user"+err);
			    ...
			    if (data===1) {
					...
					connection.query('SELECT * FROM accounts', function(err, rows, fields) {
						...
						res.render('user_group_assign', {"uList": uList, isLoggedIn: req.session.username});
						}
					});	
					}
				else{
...
app.get('/group_assign/:Uname', (req, res) => {
	if (req.session.isLoggedIn === true) {
		connection.query('SELECT * FROM ugrp where uname=?',[req.params.Uname], function(err, rows, fields) {
			if (err) {
				res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
			} else {
				var allgList=[];
				...
				for (var i = 0; i < rows.length; i++) {
					...
					connection.query('select distinct Grpname from ugrp', function(err, rows, fields) {
						// Loop check on each row
						for (var i = 0; i < rows.length; i++) {
							// Create an oubject to save current row's data
							var pp = {
	...
	console.log(Gname+ "--- "+Uname);
		switch(workonAction){
			case "add":
				connection.query(' insert into `ugrp` (`Grpname`,`Uname`) values (?,?)', [Gname, Uname], function(err, result) {
					if (err) 
		   			   res.render('index', {error: 'Add group failed:'+err, isLoggedIn: req.session.username});
					else
					   res.render('index', {error: 'Added:   '+Gname+" - "+Uname, isLoggedIn: req.session.username});
					});
				break;
				case "remove":
					connection.query(' delete from ugrp where Grpname=? and Uname=?', [Gname, Uname], function(err, result) {
						if (err) 
							res.render('index', {error: 'Remove group failed:'+err, isLoggedIn: req.session.username});
						else
							res.render('index', {error: 'Removed:   '+Gname+" - "+Uname, isLoggedIn: req.session.username});
						});
...
app.get('/kanban', (req, res) => {
	...
	connection.query('SELECT * FROM rel order by Crname', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@kanban"});
	  	} else {
	  		// Loop check on each row
	  		for (var i = 0; i < rows.length; i++) {
	  			// Create an object to save current row's data
	  	...
	  	res.render('kanban', {"appList": appList , isLoggedIn: req.session.username });
	  	}
	}); 
});
// ################################################################
...
app.post('/Create_App', (req, res) => {
	...
	connection.query('insert into app (Appname, Appdesc, AppRel, AppPBL, AppColour) values (?,?,?,?,?)', [Appname, Appdesc, AppRel, AppPBL, AppColour], function(err, result) {
			  if (err) {
...
app.get('/list_App', function(req, res) {
	  ...
	  connection.query('SELECT * FROM app', function(err, rows, fields) {
			...
			// Render index.pug page using array 
			res.render('listApp', {"appList": appList, isLoggedIn: req.session.username});
			}
	  });	
  });
// ################################################################
...
app.get('/getApp/:Appname', function(req, res) {
	  // Do the query to get data.
	  connection.query('SELECT * FROM app WHERE Appname = \'' + req.params.Appname+'\'', function(err, rows, fields) {
		  var app;
  // console.log('SELECT * FROM app WHERE Appname = \'' + req.params.Appname+'\'');
			if (err) {
				res.status(500).json({"status_code": 500,"status_message": "internal server error@detail"});
			} else {
					  ...
					  if (err) {
						  ...
						  if (data===1){  
							  ...
							  connection.query('select distinct Grpname from ugrp', function(err, rows, fields) {
								...
								//res.render('group_assign', {"allgList": allgList, "gList": gList, "Uname": req.params.Uname, isLoggedIn: req.session.username});		
								res.render('detailApp', {"allgList": allgList, "app": app, isLoggedIn: req.session.username});
							});	
...
app.post('/getApp/Update_App', (req, res) => {
  //app.post('/getApp/Update_App', function(req, res) {
	var {AppSonausr,pcreate,popen,pwip,preview,ptest,puat,pdeliver,pkiv,Appname,Appdesc1,Appdesc,AppRel,AppPBL,AppGit,AppWS,AppBuildSvr,AppBuildSvrWS,AppTestSvr,AppTestSvrWS,initGitRepo, AppColour, AppTemplateWS, AppExec ,AppBin, AppBin_user,AppGit_user,AppBin_pw, setbinpw,AppSonaQ, AppSonaS, AppSonaToken  } = req.body;
  ...
  console.log("pcreate="+pcreate+" popen="+popen);
	connection.query('update app set AppSonausr=?, pcreate=?, popen=? , pwip=?, preview=?, ptest=?, puat=?, pdeliver=?, pkiv=?, Appdesc=?,AppGit=?,AppWS=?,AppBuildSvr=?,AppBuildSvrWS=?,AppTestSvr=?,AppTestSvrWS=?,AppTemplateWS=? , AppExec=? , AppBin=? , AppBin_user=?, AppGit_user=?, AppSonaQ=? , AppSonaS=?, AppSonaToken=? where Appname=?', [AppSonausr, pcreate, popen,pwip,preview,ptest,puat,pdeliver,pkiv,Appdesc,AppGit,AppWS,AppBuildSvr,AppBuildSvrWS,AppTestSvr,AppTestSvrWS,AppTemplateWS,AppExec , AppBin, AppBin_user, AppGit_user, AppSonaQ, AppSonaS, AppSonaToken ,Appname], function(err, result) {
			  if (err) {
			   console.log("code="+ err);
				 res.render('detailApp', {error: err,"app": app, isLoggedIn: req.session.username});
			  } 
			  else {
				 res.render('index', {isLoggedIn: req.session.username}); 
			  }
		  ...
		  if (setbinpw === 'Yes') {
			connection.query('update app set AppBin_pw=? where Appname=?', [ AppBin_pw, Appname], function(err, result) {
				if (err) {
				 console.log("code="+ err);
				   res.render('detailApp', {error: err, isLoggedIn: req.session.username});
				} 
				else {
				   res.render('index', {isLoggedIn: req.session.username}); 
				}
		...
		//
		console.log("...... calling external script: git_inito_repo.sh @"+GetMyDate() );
		   var child_process = require('child_process');
		   //EXECUTE yourExternalJsFile.js
		   // "C:\\Program Files\\Git\\git-bash.exe" ==> AppExec
			 child_process.exec(AppExec+' \.\\ws\\git_init_Repo.sh '+AppWS+' '+ AppGit+' main '+ Appname +' '+ AppTemplateWS,  (error, stdout, stderr) => {
			 console.log(`${stdout}`);
			 console.log(`${stderr}`);
			 if (error !== null) {
				  ...
				  getPermit("pm",req.session.username, function(err,data){
					  if (err) {
						  ...
						  if (data===1){  
							  ...
							  var appList = [];
							  // Do the query to get data.
							  connection.query('SELECT Appname, Appdesc FROM app', function(err, rows, fields) {
									if (err) {
										res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
									} else {
										// Loop check on each row
									...
									// console.log(appList);
									// Render index.pug page using array 
									res.render('cPlan', { "appList": appList, isLoggedIn: req.session.username});
									}
								});
...
app.post('/create_plan', (req, res) => {
	const {MVPname, MVPdesc, MVPapp, MVPstartDate, MVPendDate, appList,MVPnotes} = req.body;
	if (MVPdesc===""){
		MVPdesc1="MVP Release for "+MVPname+"   [ est. start date@"+MVPstartDate+" and est. end date@"+MVPendDate+" ]";
	}
	else {
		MVPdesc1=MVPdesc;
	}
	connection.query('insert into plan (MVPnotes, MVPname, MVPdesc, MVPapp, MVPstartDate, MVPendDate, MVPiter,MVPcreator) values (?,?,?,?,?,?,?,?)', [MVPnotes, MVPname, MVPdesc1, MVPapp, MVPstartDate, MVPendDate,1, req.session.username], function(err, result) {
			  if (err) {
			   console.log("code="+ err);
			   res.render('cPlan', {"appList": appList, isLoggedIn: req.session.username, "error": err});
			  } 
			  else {
				 res.render('index', {isLoggedIn: req.session.username}); 
			  }
...
// 				1. list all planned releases 
  app.get('/list_plan',(req,res) =>{
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT MVPname, MVPapp, MVPdesc, MVPasRel, MVPXname, MVPState FROM plan', function(err, rows, fields) {
		  if (err) {
			  res.status(500).json({"status_code": 500,"status_message": "internal server error@listing plan"});
		  } else {
			  // Loop check on each row
			  for (var i = 0; i < rows.length; i++) {
				  // Create an object to save current row's data
		  ...
		  // Render index.pug page using array 
		  res.render('listPlan', {"appList": appList, isLoggedIn: req.session.username});
		  }
	});	
  });
// ################################################################
...
app.get('/getPlan/:MVPname', function(req, res) {
	// Do the query to get data.
	connection.query('SELECT MVPname,MVPapp,MVPdesc,MVPXname,MVPasRel,MVPstartDate,MVPendDate,MVPState,MVPcreator,MVPnotes FROM plan WHERE MVPname = \'' + req.params.MVPname+'\'', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@detail plan:"+req.params.MVPname});
	  	} else {
	  		// Check if the result is found or not
	  		if(rows.length==1) {
	  			// Create the object to save the data.
							...
							getPermit("pm",req.session.username, function(err,data){
								...
								if (err) {
									...
									if (data===1){  
										...
										//
										res.render('detailPlan', {"app": app, isLoggedIn: req.session.username});
									}
									else{ 
										console.log("No Permission to update. Only view",req.session);
										res.render('detailPlan_info', {"app": app, isLoggedIn: req.session.username});
									}
								}    
							});
							// ########### end getPermit ##############
	  			...
	  			// render not found page
	  			res.status(404).json({"status_code":404, "/getPlan:MVPname --> status_message": "Not found"});
	  		}
	  	}
	});
});
...
app.post('/getPlan/update_plan', (req, res) => {
	//app.post('/getApp/Update_App', function(req, res) {
	  var{MVPname , MVPapp , MVPdesc , MVPiter, MVPasRel, MVPstartDate, MVPendDate, MVPState, MVPcreator, MVPnotes, MVPnotes1,MVPnotes0}=req.body
	// console.log(req.body);
	//
	if (MVPdesc===""){
		MVPdesc="MVP Release for "+MVPname+"   [ est. start date@"+MVPstartDate+" and est. end date@"+MVPendDate+" ]";
	}
	
	connection.query('SELECT MVPnotes FROM plan WHERE MVPname = \'' + MVPname+'\'', function(err, rows, fields) {
			if(rows.length==1) {
				// Create the object to save the data.
				var app = {
				'MVPnotes':rows[0].MVPnotes	}
			}
				MVPnotes2=stamp(req.session.username,MVPState)+"\r"+MVPnotes1+"\r"+app.MVPnotes;	

				connection.query('update plan set MVPdesc=? ,MVPstartDate=?, MVPendDate=?, MVPnotes=? where MVPname=?', [MVPdesc, MVPstartDate, MVPendDate, MVPnotes2, MVPname], function(err, result) {
						if (err) {
						console.log("code="+ err);
						res.render('cPlan', {isLoggedIn: req.session.username, "error@plan update": err});
						} 
						else {
							res.render('index', {isLoggedIn: req.session.username}); 
						}
...
app.get('/list_App_4rel', function(req, res) {
	...
	connection.query('SELECT * FROM app', function(err, rows, fields) {
	  	...
	  	// Render index.pug page using array 
	  	res.render('cRelListApp', {"appList": appList, isLoggedIn: req.session.username});
	  	}
	});
});
// ################################################################
	  		...
	  		if(rows.length==1) {
				...
				getPermit(app.pcreate,req.session.username, function(err,data){
					...
					if (err) {
						...
						if (data===1){  
							// render the details.plug page.
							res.render('cRel', {"app": app, isLoggedIn: req.session.username});
						}
						else{ 
						    console.log("No Permission to proceed",req.session);
							res.render('noPermit', {isLoggedIn: req.session.username});
				...
				// ########### end getPermit ##############
	  			...
	  			// render not found page
	  			res.status(404).json({"create rel: mstatus_code":404, "status_message": "Not found"});
	  		}
	  	}
	});
});
...
app.post('/createRel/Create_Rel', (req, res) => {
	var {Crname, Crdesc, Crapp, CrRelType, CrDeployType, CrColour,workonaction} = req.body;
	var myDate = new Date();
	var dummy,dummy1;
	dummy=0;
	dummy='';
	...
	connection.query('insert into rel (Crname, Crdesc, Crapp, CrRelItr, CrState, Crcreator, Crowner, CrRelType, CrDeployType, CrColour) values (?,?,?,?,?,?,?,?,?,?)', [Crname, Crdesc, Crapp, 0, 'open',req.session.username, req.session.username, CrRelType, CrDeployType, CrColour], function(err, result) {
			  if (err) {
			   console.log("code="+ err);
...
app.get('/approve_rel', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where Crstate="open"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array 
	  	res.render('listRelapprove', {"appList": appList, isLoggedIn: req.session.username, opType: "Approve"});
		}
	});	
});
// ################################################################
...
app.get('/getRel/:Appname', function(req, res) {
	// Do the query to get data.
	connection.query('SELECT * FROM rel WHERE Crname = \'' + req.params.Appname+'\'', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@detail"});
	  	} else {
	  		// Check if the result is found or not
	  		if(rows.length==1) {
	  			...
	  			var app = {
					...
					connection.query('SELECT * FROM app WHERE Appname =? ',[app.Crapp], function(err, rows) {
						var pp={
								'popen':rows[0].popen,
								'pwip':rows[0].pwip,
								'preview':rows[0].preview,
							...
							getPermit(pp.popen,req.session.username, function(err,data){
								...
								if (err) {
									...
									if (data===1){  
										...
										// to determine what action is taken on the release request
										//
										res.render('approveRel', {"app": app, isLoggedIn: req.session.username});
									}
									else{ 
										console.log("No Permission to proceed",req.session);
										res.render('noPermit', {isLoggedIn: req.session.username});
	  			...
	  			// render not found page
	  			res.status(404).json({"status_code":404, "/getRel:Appname --> status_message": "Not found"});
	  		}
	  	}
	});
});
...
app.post('/getRel/approve_rel', (req, res) => {
	//app.post('/getApp/Update_App', function(req, res) {
	  var{Crname , Crdesc , Crapp , CrRelItr, CrCodebase, CrState, CrRelName, CrNote, CrDevbranch, CrRelbranch, CrRelType,CrDeployType, Crcreator, Crowner,CrDevInst, CrRelInst, CrColour, workonaction}=req.body
	// console.log(req.body);
	//
	// 1. construct release_label
	//

		connection.query('SELECT AppPrdBL,AppPbugrel,Appname, AppRel, AppPBL, AppGit, AppBuildSvr, AppWS, AppColour FROM app WHERE Appname = ?',[Crapp] , function(err, result) {
		  if (err) {
				  res.status(500).json({"status_code": 500,"status_message": "internal server error@approve RR"});
			  } else {
					  // Create an object to save current row's data
			...
			if (workonaction=="approve"){
				...
				//
				var srcBranch="main";
				if (CrRelType=="pbug") {   // pbug for production bug fix
					...
					console.log("...... calling external script: git_create_branch.sh @"+GetMyDate() );
					var child_process = require('child_process');
					//EXECUTE yourExternalJsFile.js
					child_process.execSync('"C:\\Program Files\\Git\\git-bash.exe" \.\\ws\\git_create_branch.sh '+myAppWS+' '+ myAppGit+' '+srcBranch+' Dev_'+ Crname +' ' + myAppname + ' NoLock', (error, stdout, stderr) => {
						console.log(`${stdout}`);
						console.log(`${stderr}`);
						if (error !== null) {
							console.log(`exec error: ${error}`);
				...
				connection.query('update app set AppRel=? where Appname=?', [myAppRel,Crapp], function(err, result) {
								if (err) {
							console.log("code="+ err);
							res.render('approveRel', {error: err, isLoggedIn: req.session.username});
							//res.render('index', {error: err});
							} 
							else
								console.log("AppRel :"+myAppRel);
				});
			}	     
			else
				{
			...
			// update rel cr
			connection.query('update rel set    Crname=?,   Crdesc=?,   CrRelItr=?,   CrCodebase=?,   CrState=?,   CrRelName=?,   CrNote=?,   CrDevbranch=?,   CrRelbranch=?,   CrRelType=?,   CrDeployType=?,    Crowner=? , CrDevInst=? where Crname=?', [   Crname,   Crdesc,   CrRelItr,   CrCodebase,   CrState,   CrRelName,   CrNote,   CrDevbranch,   CrRelbranch,   CrRelType,   CrDeployType,    Crowner, CrDevInst , oCrname], function(err, result) {
	  	...
	  	if (err) {
	  		...
	  		if(rows.length==1) {
	  			...
	  			var app = {
					...
					res.render('detailRel_info', {"app": app , isLoggedIn: req.session.username });
	  		} else {
	  			// render not found page
	  			res.status(404).json({"status_code":404, "rel info:Appname --> status_message": "Not found"});
	  		}
	  	}
	});
});
...
app.get('/list_cr', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState= "wip"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
			...
			res.render('listRelCr', {"appList": appList , isLoggedIn: req.session.username , opType: "Review"});
	  	}
	});
});
// ################################################################
							...
							getPermit(pp.pwip,req.session.username, function(err,data){
								...
								if (err) {
									...
									if (data===1){  
										...
										// to determine what action is taken on the release request
										//
										  if (app.CrCodebase===pp.AppPBL || app.CrRelType=="pbug" ) {
										    ...
										    aa="code ok";
	 									    ...
	 									    res.render('detailRel', { "app": app , msg:aa , isLoggedIn: req.session.username }); 
									}
									
									else{ 
										console.log("No Permission to proceed. Readonly allowed",req.session);
											res.render('detailRel_info', { "app": app , msg: aa ,isLoggedIn: req.session.username })
									}
								}    
							});
							// ########### end getPermit ##############
...
app.post('/getRelAct/workon_Rel', (req, res) => {
	  var{Crsonastatus, Crname , Crdesc , Crdesc1, Crapp , CrRelItr, CrCodebase, CrState, CrRelName, CrNote, CrDevbranch, CrRelbranch, CrRelType,CrDeployType, Crcreator, Crowner,CrDevInst, CrRelInst, CrColour, CrNote1,workonAction, newPBL}=req.body
	// #############################################################
	// check what action is requested
	// #############################################################

		if (workonAction === "develop" || workonAction==="mergecode") { // ##### to Develop or update code merge status ######
			connection.query('SELECT * FROM rel WHERE Crname =? ',[Crname], function(err, rows) {
				var pp={
						'CrNote':rows[0].CrNote}
				// append notes
				...
				if (workonAction==="mergecode"){ // code merge status action
					console.log("merge code triggered.");
					connection.query('update rel set CrCodebase=?, CrRelType=?, CrDeployType=?, CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [ newPBL, CrRelType, CrDeployType, CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {			
						if (err) {
					...
					connection.query('update rel set CrRelType=?, CrDeployType=?, CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [ CrRelType, CrDeployType, CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {			
						if (err) {
						console.log("fail to wip state rel update: "+ err);
						//res.render('approveRel', {error: err});
							res.render('index', {error: err, isLoggedIn: req.session.username});
			...
			// console.log("====> "+ Crapp);  		
			connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
				var pp={
						'Appname':rows[0].Appname,
						'Appdesc':rows[0].Appdesc,
						'AppRel':rows[0].AppRel,
					...
					connection.query('SELECT * FROM rel WHERE Crname =? ',[Crname], function(err, rows) {
						var crp={
								'CrNote':rows[0].CrNote }  // some how the form passing did not send back CrNote
						...
						// # ##############################################################
								...
								// test external calling of script
									console.log("...... calling external script: git_create_branch.sh @"+GetMyDate() +" for "+CrRelbranch);
									var child_process = require('child_process');
									//EXECUTE yourExternalJsFile.js
									// "C:\\Program Files\\Git\\git-bash.exe" ==> AppExec
									child_process.execSync(pp.AppExec+' \.\\ws\\git_create_branch.sh '+pp.AppWS+' '+ pp.AppGit+' '+ Crname +' '+ CrRelbranch +' ' + Crapp + ' lock', (error, stdout, stderr) => {
										console.log(`${stdout}`);
										console.log(`${stderr}`);
										if (error !== null) {
											console.log(`exec error: ${error}`);
										...
										// echo "App              =  $7"
										// echo "Sona user        =  $8"
										console.log(pp.AppExec+' \.\\ws\\iscan_sonarqube.sh '+pp.AppWS+' '+ CrRelbranch+' '+pp.AppSonaQ +' '+ pp.AppSonaS +' '+ pp.AppSonaToken+' '+pp.AppGit+' ' + Crapp+' '+pp.AppSonausr);
										child_process.exec(pp.AppExec+' \.\\ws\\iscan_sonarqube.sh '+pp.AppWS+' '+ CrRelbranch+' '+pp.AppSonaQ +' '+ pp.AppSonaS +' '+ pp.AppSonaToken+' '+pp.AppGit+' ' + Crapp+' '+pp.AppSonausr , (error, stdout, stderr) => {
											console.log(`${stdout}`);
											if (error !== null) {
												console.log(`exec error: ${error}`);
										}
								...
								CrState="review"; // set the state to WIP
										// append notes
								var sonamsg=stamp("system-messenger",CrState)+"\n Sonarqube Scan started . \n";		
								var KrNote=sonamsg+stamp(req.session.username,CrState)+"\r"+CrNote1;
								...
								connection.query('update rel set Crsonastatus=?, CrRelType=?, CrDeployType=?, CrRelName=?, CrRelbranch=?, CrRelInst=?,  CrRelItr=?, CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [ "", CrRelType, CrDeployType, CrRelName, CrRelbranch, CrRelInst, CrRelItr, CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {
									if (err) {
									console.log("fail to wip state rel update: "+ err);
									//res.render('approveRel', {error: err});
										res.render('index', {error: err, isLoggedIn: req.session.username});
...
app.get('/list_cr4Review', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState= "review"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array {isLoggedIn: req.session.username}
	  	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username,opType: "Test" });
	  	}
	});
});
// ################################################################
						...
						var pp={
							...
							getPermit(pointer,req.session.username, function(err,data){
								...
								if (err) {
									...
									//
									var planList= [];
									connection.query('SELECT MVPname FROM plan', function(err, rows, fields) {  
										  // Render index.pug page using array {isLoggedIn: req.session.username}
										  ...
										  // prep the list of MVP Plan
											...
											if (app.CrCodebase===pp.AppPBL || CrRelType=="pbug") {
											...
											sonarResult("localhost:9000", "admin", "password", app.CrRelbranch,function(sonastatus) {
											console.log("callee received: "+sonastatus);
											if (sonastatus.includes("completed")){
												 app.Crsonastatus=pp.AppSonaQ+"dashboard?id="+app.CrRelbranch;
											// update the CR table
													connection.query('update rel set Crsonastatus=? WHERE Crname = ?', [app.Crsonastatus, app.Crname], function( err, result) 
													{
														if (err) 
															console.log(err);
													});

				  						  		  res.render('detailRel_lead', {"planList": planList, "app": app , msg:aa,  isLoggedIn: req.session.username });
												}
											else
 											   res.render('detailRel_lead', {"planList": planList, "app": app , msg:aa,  isLoggedIn: req.session.username });
											});
...
app.post('/getRelAct4review/workon_Rel', (req, res) => {
	var{oCrState, Crname , Crdesc , Crdesc1, Crapp , CrRelItr, CrCodebase, CrState, CrRelName, CrNote, CrDevbranch, CrRelbranch, CrRelType,CrDeployType, Crcreator, Crowner,CrDevInst, CrRelInst, CrColour, CrNote1,workonAction,CrXname, CrXplan}=req.body
  // #############################################################
  // check what action is requested
  // #############################################################
	...
	switch(workonAction){
						...
						console.log("-->"+workonAction);
						  ...
						  connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							  var pp={
									 ...
									 connection.query('SELECT * FROM rel WHERE Crname =? ',[Crname], function(err, rows) {
										  var crp={
												  'iCrNote':rows[0].CrNote }  // some how the form passing did not send back CrNote
										  if (Crdesc1==="" || Crdesc1 === null) {
											...
											CrNote=KrNote+"\r"+crp.iCrNote;
														   // Crdesc can be changed
											// update rel request
											connection.query('update rel set CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [  CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {
												if (err) {
													...
													// update App
													connection.query('update app set AppSezone=? where Appname=?', [ null ,Crapp], function(err, result) {
														if (err) {
		...
		case  "review":
			connection.query('SELECT * FROM rel WHERE Crname =? ',[Crname], function(err, rows) {
				...
				console.log('here ='+CrState);
				connection.query('update rel set CrState=?, Crdesc=?, CrNote=?, Crowner=? ,CrXplan=? where Crname=?', [ CrState, Crdesc, CrNote, req.session.username, CrXplan, Crname], function(err, result) {			
					if (err) {
						...
						connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							var pp={
								  ...
								  if (pp.AppSezone === Crname) {
										...
										connection.query('update app set AppSezone=?  where Appname=?', [ null , Crapp], function(err, result) {
											if (err) {
												console.log("fail to transit out of Sezone state app update: "+ err);
												res.render('index', {error: err, isLoggedIn: req.session.username});
											}
											else {
												res.render('index', {isLoggedIn: req.session.username}); 
						  ...
						  connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							  var pp={
									 ...
									 if (CrRelType==="pbug") { 
										...
										console.log("...... calling external script: git_merge_main.sh @"+GetMyDate() +" for "+CrRelbranch +"@"+Crapp);
										var child_process = require('child_process');
										//
										//
											temp=pp.AppExec+' \.\\ws\\git_merge_main.sh '+pp.AppWS+' '+pp.AppGit+' '+CrRelbranch+' '+"main"+' '+Crapp;
											console.log(temp); 
										/*
											echo "Workpath   : $1"
											echo "Git Repo   : $2"
										...
										*/
										...
										child_process.execSync(pp.AppExec+' \.\\ws\\git_merge_main.sh '+pp.AppWS+' '+pp.AppGit+' '+CrRelbranch+' '+"main"+' '+Crapp , (error, stdout, stderr) => {
											console.log(`${stdout}`);
											console.log(`${stderr}`);
											if (error !== null) {
												console.log(`exec error: ${error}`);
									  ...
									  CrCodebase=pp.AppPBL;
											// update rel request
											connection.query('update rel set CrCodebase=?, CrXname=?, CrState=?, Crdesc=?, CrNote=?, Crowner=? , CrXplan=? where Crname=?', [ CrCodebase, CrXname, CrState, Crdesc, CrNote, req.session.username,CrXplan, Crname], function(err, result) {
												if (err) {
													...
													//
													// update App
													connection.query('update app set AppPBL=?, AppPbugrel=?, AppMVPrel=?  where Appname=?', [ pp.AppPBL, pp.AppPbugrel, pp.AppMVPrel, Crapp], function(err, result) {
														if (err) {
															console.log("fail to uat state app update: "+ err);
															res.render('index', {error: err, isLoggedIn: req.session.username});
														}
														else {
															connection.query('update plan set MVPasRel=?, MVPState=? , MVPXname=?  where MVPname=?', [ CrRelbranch, CrState, CrXname, CrXplan], function(err, result) {
																if (err){
																  res.render('index', {error: err, isLoggedIn: req.session.username});
																}
																  else {
																	if (CrRelType=="pbug")
																		res.render('manualMerge',{CrRelbranch: CrRelbranch, isLoggedIn: req.session.username}); 
																	else
																		res.render('index', {isLoggedIn: req.session.username}); 
																}
															});
						  ...
						  connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							  var pp={
								  ...
								  //	
									  ...
									  // # ##############################################################
									  // test external calling of script
									  console.log("...... calling external script: ibuild_pipeline.sh @"+GetMyDate() +" for "+CrRelbranch +"@"+Crapp);
									  var child_process = require('child_process');
									  /*
										  echo "workspace local  =  $1"
										  echo "release_branch   =  $2"
									  ...
									  */
									  ...
									  child_process.execSync(pp.AppExec+' \.\\ws\\ibuild_pipeline.sh '+pp.AppWS+' '+ CrRelbranch+' '+pp.AppBuildSvr+' '+pp.AppBuildSvrWS +' '+ pp.AppBin+' '+pp.AppGit+' '+ Crapp +' '+ pp.AppBin_user+' '+ pp.AppBin_pw+' '+ pp.AppTestSvr+' '+pp.AppTestSvrWS, (error, stdout, stderr) => {
										  console.log(`${stdout}`);
										  console.log(`${stderr}`);
										  if (error !== null) {
											  console.log(`exec error: ${error}`);
						  ...
						  connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							  var pp={
								    ...
								    if (pp.AppSezone !== "" && pp.AppSezone !== null) {
										res.render('Sezone',{CrXname: CrXname, AppSezone: pp.AppSezone, isLoggedIn: req.session.username}); 
									}
									else { 
									 	   console.log('The current production runtime is the latest (is the greatest) :'+pp.AppProduction+'('+pp.AppPrdBL+') vs ' + CrXname);
										   if (pp.AppPrdBL >= CrXname ) {
											res.render('stale', {AppProduction: pp.AppProduction, AppPrdBL:pp.AppPrdBL, CrXname: CrXname, isLoggedIn: req.session.username});
										   }
										   else {													CrState="deliver";
													console.log('here ='+CrState);
													console.log("CrRelType="+CrRelType);
													connection.query('SELECT * FROM rel WHERE Crname =? ',[Crname], function(err, rows) {
														var crp={
														...
														if (Crdesc1==="" || Crdesc1 === null) {
															...
															connection.query('update rel set  CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [  CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {
																if (err) {
																	...
																	// update App
																	connection.query('update app set AppSezone=?  where Appname=?', [ Crname , Crapp], function(err, result) {
																		if (err) {
																			console.log("fail to uat state app update: "+ err);
																			res.render('index', {error: err, isLoggedIn: req.session.username});
																		}
			...
			// check if Crname is the request  in the serialised zone
						  ...
						  connection.query('SELECT * FROM app WHERE Appname =? ',[Crapp], function(err, rows) {
							  var pp={
									  ...
									  // test external calling of script		  	 
								    if (pp.AppSezone !== Crname) {
										res.render('Sezone',{Crname: Crname, AppSezone: pp.AppSezone, isLoggedIn: req.session.username}); 
									}
									else { 
									 CrState="close";
									 console.log('here ='+CrState);
										  ...
										  if (Crdesc1==="" || Crdesc1 === null) {
											...
											connection.query('update rel set  CrState=?, Crdesc=?, CrNote=?, Crowner=? where Crname=?', [  CrState, Crdesc, CrNote, req.session.username, Crname], function(err, result) {
												if (err) {
													...
													// update App
													connection.query('update app set AppSezone=?,AppProduction=? ,AppPrdBL=? where Appname=?', [ null , CrXname,CrRelbranch,Crapp], function(err, result) {
														if (err) {
															console.log("fail to uat state app update: "+ err);
															res.render('index', {error: err, isLoggedIn: req.session.username});
														}
...
app.get('/list_cr4UAT', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState="test" ', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array {isLoggedIn: req.session.username}
	  	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username , opType:"UAT"});
	  	}
	});
});
//
...
app.get('/list_cr4StatusUpdate', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState="uat" or CrState="deliver"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array {isLoggedIn: req.session.username}
	  	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username , opType:"State Update"});
	  	}
	});
});
//
...
app.get('/list_cr4Revive', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState= "kiv"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	 	...
	 	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username, opType:"Revive" }); 
	  	}
	});
});
//
...
app.get('/list_retired', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState= "imo" or Crstate="reject" or Crstate="kiv"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array {isLoggedIn: req.session.username}
	  	//res.render('listRelCr4Revive', {"appList": appList , isLoggedIn: req.session.username});
	 	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username, opType:"Viewing Purpose" }); 
	  	}
	});
});
//
...
app.get('/close_cr', function(req, res) {
	var appList = [];
	// Do the query to get data.
	connection.query('SELECT * FROM rel where CrState= "deliver"', function(err, rows, fields) {
	  	if (err) {
	  		res.status(500).json({"status_code": 500,"status_message": "internal server error@listing"});
	  	} else {
	  		// Loop check on each row
	  	...
	  	// Render index.pug page using array {isLoggedIn: req.session.username}
	  	//res.render('listRelCr4Revive', {"appList": appList , isLoggedIn: req.session.username});
	 	res.render('listRelCr4Review', {"appList": appList , isLoggedIn: req.session.username, opType:"Close" }); 
	  	}
	});
});
// ################################################################
